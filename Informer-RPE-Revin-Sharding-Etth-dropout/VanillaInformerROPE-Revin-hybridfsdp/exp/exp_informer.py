...
7. channel_mix_size threaded through to EncoderLayer
"""
...
# =============================================================================
# FIX: GradScaler import — works with both old and new PyTorch
# =============================================================================
try:
    # PyTorch 2.0+
    from torch.amp import GradScaler
    _GRADSCALER_DEVICE = True   # new API needs device='cuda'
except ImportError:
    # PyTorch 1.x
...
class Exp_Informer(Exp_Basic):
    ...
    def _wrap_model_with_fsdp(self, model):
        ...
        # Auto-wrap: each EncoderLayer / DecoderLayer is one FSDP unit
    ...
    def _select_optimizer(self):
        """
        Select optimizer — AdamW with decoupled weight decay.

        Splits parameters into decay (weights) and no-decay (bias, norm, affine).
        FSDP compatible: use_orig_params=True preserves original parameter name suffixes.

        CRITICAL FIX: The previous filter excluded 'embedding' which caught ROPE
        learnable parameters (rpe.sin_embed, rpe.cos_embed etc.) — 102M+ params
        with zero weight decay, causing severe overfitting. Now we only exclude
        biases, LayerNorm/BatchNorm params, and RevIN affine params.
        """
        decay_params = []
        no_decay_params = []
        ...
        for name, param in self.model.named_parameters():
            if not param.requires_grad:
                continue
            # Only exclude: biases, LayerNorm/BatchNorm, RevIN affine params
            # DO NOT exclude ROPE embeddings — they are 100M+ learnable params
            if any(nd in name.lower() for nd in ['bias', 'layernorm', 'batchnorm',
        ...
        if self._should_print():
            ...
            print(f"  AdamW param groups: {n_decay:,} decay, {n_no_decay:,} no-decay")
    ...
    def train(self, setting):
        ...
        # =====================================================================
        # LR SCHEDULER: Linear warmup + Cosine annealing (step-level)
        # Replaces the old epoch-level adjust_learning_rate() call.
        # =====================================================================
        ...
        if lradj == 'cosine':
            ...
            if self._should_print():
                print(f"  LR Schedule: Cosine with linear warmup")
        ...
        # FIX: GradScaler API for PyTorch 2.0+
        if use_scaler:
            if _GRADSCALER_DEVICE:
        ...
        if self._should_print():
            ...
            print(f"  Optimizer: AdamW (wd={getattr(self.args, 'weight_decay', 0.01)})")
            print(f"  LR schedule: {lradj}" + (f" (warmup={warmup_steps})" if scheduler else ""))
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, batch in enumerate(data_iter):
                ...
                if should_sync:
                    ...
                    # Step LR scheduler (per optimizer step, not per iteration)
            ...
            # Cosine scheduler handles LR per-step; only use epoch-level
            # adjust for legacy schedule types (type1, type2).
            # =================================================================
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        # =====================================================================
        # CRITICAL FIX: Match target dtype to model output dtype.
        #
        # With FSDP MixedPrecision(param_dtype=bfloat16), model outputs are
        # BF16 but batch_y is FP32 from the .float() cast above.  If we
        # compute loss between BF16 pred and FP32 true, MSELoss promotes to
        # FP32 → loss is FP32 → backward() produces FP32 gradients → FSDP
        # expects BF16 gradients → RuntimeError:
        #   "Found dtype Float but expected BFloat16"
        #
        # Fix: cast target to match model output dtype so the entire loss
        # computation (and its backward) stays in the model's native dtype.
        # =====================================================================
    ...
    def _process_one_batch_prefetched(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        """
        # Data is already on GPU from prefetcher, just ensure correct dtype
        batch_x = batch_x.float()
        batch_y = batch_y.float()  # Already on GPU
        ...
        # =====================================================================
        # CRITICAL FIX: Match target dtype to model output dtype.
        # Same issue as _process_one_batch — see detailed comment there.
        # =====================================================================
    ...
    def test(self, setting, load=False):
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                # .float() for consistent numpy conversion later
        ...
        if self._should_print():
            ...
            if hasattr(test_data, 'scaler') and test_data.scaler is not None:
                np.save(f'{folder}pred.npy', test_data.scaler.inverse_transform(preds_to_save))
                np.save(f'{folder}true.npy', test_data.scaler.inverse_transform(trues_to_save))
            else:
