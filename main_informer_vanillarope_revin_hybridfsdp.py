...
def create_args():
    ...
    args.pred_len = 96 * args.channel_period
    ...
    args.weight_decay = 0.01  # AdamW decoupled weight decay
    args.loss = 'mse'
    args.lradj = 'cosine'
    args.warmup_ratio = 0.05  # Linear warmup over 5% of total steps
    args.use_amp = True
...
def print_config(args):
    ...
    print(f"Optimizer: AdamW (weight_decay={getattr(args, 'weight_decay', 0.01)})")
    print(f"LR Schedule: {getattr(args, 'lradj', 'type1')} (warmup_ratio={getattr(args, 'warmup_ratio', 0.0)})")
    print(f"LR: {args.learning_rate}, Patience: {args.patience}")
    print(f"{'=' * 60}\\n")
